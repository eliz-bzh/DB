...
--DROP TRIGGER IF EXISTS Equipment_Update;
--DROP TRIGGER IF EXISTS StreetCus_InsertDelete;
--GO
...
--CREATE TRIGGER Equipment_Insert ON Equipment AFTER INSERT
--AS
--BEGIN
...
--CREATE TRIGGER Equipment_Update ON Equipment AFTER UPDATE
--AS
--BEGIN
--	SELECT 'deleted';
--	SELECT * FROM deleted;
...
--CREATE TRIGGER StreetCus_InsertDelete ON StreetCus
--AFTER Insert, Delete
--AS
--BEGIN
--	print 'Элемент добавлен или удалён'
--END
--GO
...
--UPDATE Equipment SET [name] = N'Гиря православная'
--WHERE id_e = 2
...
--вывод назв. и тип оборудования
CREATE VIEW View1
AS
SELECT e.[name] AS 'Название оборудования', te.[name] AS 'Тип оборудования'
FROM Equipment e
JOIN TypeEquipment te ON te.id_te = e.id_te
GO

--вывод количества определённых типов оборудования
CREATE VIEW View2
AS
SELECT te.[name] AS 'Тип оборудования', COUNT(*) AS 'Кол-во типов оборудования'
FROM Equipment e
JOIN TypeEquipment te ON e.id_te = te.id_te
GROUP BY te.[name]
GO

--вывод всего об посещениях, которые будут
CREATE VIEW View3
AS
SELECT c.[name] AS Имя, c.[surname] AS Фамилия, c.email AS 'E-mail', CONCAT(stcl.[name], ', ', cl.numberHouse) AS Адрес, v.date AS Дата, tp.[name] AS Помещение
FROM Visit v
JOIN Customer c ON v.id_c = c.id_c
JOIN Club cl ON cl.id_cl = v.id_cl
JOIN StreetClub stcl ON stcl.id_stcl = cl.id_stcl
...
CREATE VIEW View4
AS
SELECT COUNT(invitation) AS 'Кол-во пригласительных'
FROM Customer
WHERE invitation = 1;
GO

--подсчёт одинаковых фамилий среди посетителей
CREATE VIEW View5
AS
SELECT Customer.surname AS Фамилия, COUNT(*) AS 'Кол-во фамилий'
FROM Customer

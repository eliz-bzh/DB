...
--вывод улиц посетителей которые заканчтваются на а
SELECT name AS Street
FROM StreetCus
WHERE name LIKE '%а';

--вывод абонементов цена которых больше или равно 100
SELECT price AS 'Price subscription', amountVisit AS 'Amount visited', validity AS 'Validity period'
FROM Subscription
WHERE price >= 100;

--вывод типов помещений, где содержится слово Зал
SELECT name AS 	'Type Premise'
FROM TypePremise
WHERE name LIKE '%Зал%'
ORDER BY name;

--вывод клубов на улицах Немига и Зыбицкая
SELECT phone AS Phone, numberHouse AS 'House number', StreetClub.name AS Street
FROM Club
JOIN StreetClub ON Club.id_stcl = StreetClub.id_stcl
WHERE StreetClub.name IN ('Немига', 'Зыбицкая')
ORDER BY numberHouse;

--вывод посетителей без приглашения и сортированные по убыванию номера квартир
SELECT Customer.name AS Name, surname AS Surname, patronymic AS Patronymic, 
numberFlat AS Flat, invitation AS Invitation, StreetCus.name AS Street
FROM Customer
JOIN StreetCus ON Customer.id_stc = StreetCus.id_stc
WHERE invitation = 0
ORDER BY numberFlat DESC;

--вывод посещений с 0 до 14 часов
SELECT Customer.surname AS 'Surname customer', Customer.name AS 'Name customer', Customer.phone AS 'Phone customer',
date AS 'Date visited', time AS 'Time visited'
FROM Visit
JOIN Customer ON Customer.id_c = Visit.id_c
WHERE DATEPART(HOUR, time) BETWEEN 0 AND 14;

--вывод всех улиц, где живут посетители и находятся клубы 
SELECT name AS Streets FROM
(SELECT name FROM StreetCus
UNION
SELECT name FROM StreetClub) AS Street
ORDER BY name;

--вывод оборудования, если имеется слово Степпер или тип оборудования Атлетика
SELECT Equipment.name AS Equipment, TypeEquipment.name AS 'Type Equipment'
FROM Equipment
JOIN TypeEquipment ON Equipment.id_te = TypeEquipment.id_te
WHERE Equipment.name LIKE '%Степпер%'
OR TypeEquipment.name = 'Атлетика';

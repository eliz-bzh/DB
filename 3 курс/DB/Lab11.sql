...
--вывод названий улиц клубов встречающиеся больше 2 раз
...
--вывод посещений, которые будут в текущем месяце текущего года
SELECT Visit.date AS Date, Visit.time AS Time,
Customer.name AS 'Customer Name', Customer.surname AS 'Customer Surname', Customer.patronymic AS 'Customer Patronymic', Customer.phone AS 'Customer Phone',
StreetClub.name AS 'Street Club', Club.numberHouse AS 'Number house of Club'
FROM Visit
...
JOIN Club ON Visit.id_cl =	Club.id_cl
JOIN StreetClub ON Club.id_stcl = StreetClub.id_stcl
WHERE DATEPART(MONTH,Visit.date) = DATEPART(MONTH,GETDATE())
AND DATEPART(YEAR,Visit.date) = DATEPART(YEAR,GETDATE())
ORDER BY Visit.date;

--вывод посетителей с колическов месяцев для посещения клуба
...
JOIN Customer ON Customer.id_c = Subscription_Cus.id_c
JOIN Subscription ON Subscription.id_s = Subscription_Cus.id_s
WHERE DATEDIFF(MONTH, GETDATE(), Subscription.validity) != 0;

--3 строковые
--вывод посетителей и принадлежавших им абонементы, где номер поситителя начинается с +37533
...
CONCAT(Subscription.price, ' Byn') AS 'Price subscription', Subscription.amountVisit AS 'Amount visited', 
Subscription.validity AS 'Validity period'
FROM Subscription_Cus
JOIN Customer ON Customer.id_c = Subscription_Cus.id_c
JOIN Subscription ON Subscription.id_s = Subscription_Cus.id_s
WHERE Customer.phone LIKE '+37533%'
ORDER BY Subscription.price;

--вывод сокращённого названия помещения с принадлежавшим им оборудованием
SELECT CONCAT(LOWER(LEFT(TypePremise.name, 3)), '.') AS Primise, Equipment.name AS 'Equipment',TypeEquipment.name AS 'Type of equipment'
FROM Equipment
JOIN Premise ON Premise.id_p = Equipment.id_p
JOIN TypePremise ON Premise.id_tp = TypePremise.id_tp
JOIN TypeEquipment ON TypeEquipment.id_te = Equipment.id_te
ORDER BY Primise;

--вывод помещения и принадлежавшее оборудование с зашифрованным типом
SELECT TypePremise.name AS Primise, Equipment.name AS 'Equipment', LOWER(REVERSE(TypeEquipment.name)) AS 'Type of equipment'
FROM Equipment
JOIN Premise ON Premise.id_p = Equipment.id_p
JOIN TypePremise ON Premise.id_tp = TypePremise.id_tp
JOIN TypeEquipment ON TypeEquipment.id_te = Equipment.id_te
...
--3 математические
--вывод посещений и абонементов с огруглённой ценой
SELECT Customer.name AS Name, Customer.surname AS Surname, Customer.patronymic AS Patronymic,
Customer.email AS 'E-mail', Customer.phone AS Phone, 
REPLACE(ROUND(Subscription.price, 0), '.00', ' Byn') AS 'Price subscription', Subscription.amountVisit AS 'Amount visited'
FROM Subscription_Cus
JOIN Customer ON Customer.id_c = Subscription_Cus.id_c
JOIN Subscription ON Subscription.id_s = Subscription_Cus.id_s;

--вывод посетителей с количеством посещения
SELECT Customer.name AS Name, Customer.surname AS Surname, Customer.patronymic AS Patronymic,
Customer.email AS 'E-mail', Customer.phone AS Phone, 
...
--вывод посещений и абонементов с увеличенным кол-вом посещений на N-раз, где N-квадрат кол-ва посещений
SELECT Customer.name AS Name, Customer.surname AS Surname, Customer.patronymic AS Patronymic,
Customer.email AS 'E-mail', Customer.phone AS Phone, 
REPLACE(ROUND(Subscription.price, 0), '.00', ' Byn') AS 'Price subscription', SQUARE(Subscription.amountVisit) AS 'Amount visited'
FROM Subscription_Cus
JOIN Customer ON Customer.id_c = Subscription_Cus.id_c
JOIN Subscription ON Subscription.id_s = Subscription_Cus.id_s
WHERE Subscription.amountVisit != 0;
